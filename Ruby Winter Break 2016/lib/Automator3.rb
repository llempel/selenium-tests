...
###BrowserMob
server = BrowserMob::Proxy::Server.new('/Users/llempel/Documents/browsermob-proxy-2.1.1/bin/browsermob-proxy') #=> #<BrowserMob::Proxy::Server:0x000001022c6ea8 ...>
  ...
  drivers.each do |browser|
    ...
    begin
      if page_loaded
        ...
        elsif driver_index == 1
          browser = Selenium::WebDriver.for(:chrome, :desired_capabilities => caps, :switches => %w[--user-agent=Mozilla/5.0(iPad; U; CPU iPhone OS 3_2 like Mac OS X; en-us) AppleWebKit/531.21.10 (KHTML, like Gecko) Version/4.0.4 Mobile/7B314 Safari/531.21.10])
          browser.manage.window.resize_to(768, 1024)
        else
          browser = Selenium::WebDriver.for(:chrome, :desired_capabilities => caps, :switches => %w[--user-agent=Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13B143 Safari/601.1])
          browser.manage.window.resize_to(375, 667)
        end
        next
      end
